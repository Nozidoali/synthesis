...
void WriteDotNtk(Abc_Ntk_t * pNtk, Vec_Ptr_t * vNodes, char * pFileName, int fGateNames)
{
    ...
    if ( fHasCos )
    {
        ...
        Vec_PtrForEachEntry( Abc_Obj_t *, vNodes, pNode, i )
        {
            ...
            fprintf( pFile, "  Node%d [label = \"%d%s\"",
                pNode->Id,
                // (Abc_ObjIsBi(pNode)? Abc_ObjName(Abc_ObjFanout0(pNode)):Abc_ObjName(pNode)),
                pNode->Id,
                (Abc_ObjIsBi(pNode)? "_in":"") );
    ...
    for ( Level = LevelMax - fHasCos; Level >= LevelMin && Level > 0; Level-- )
    {
        ...
        Vec_PtrForEachEntry( Abc_Obj_t *, vNodes, pNode, i )
        {
            ...
            if ( Abc_NtkIsStrash(pNtk) ) {}
                ...
                assert( 0 );
                // pSopString = NtkPrintSop(Mio_GateReadSop((Mio_Gate_t *)pNode->pData));
            else
...
void Ckt_Visualize(Abc_Ntk_t * pAbcNtk, string fileName)
{
    ...
    assert( Abc_NtkIsStrash(pAbcNtk) || Abc_NtkIsLogic(pAbcNtk) );
    if ( Abc_NtkIsStrash(pAbcNtk) && Abc_NtkGetChoiceNum(pAbcNtk) )
    {
